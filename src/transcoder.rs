...
use ffmpeg_next::{
    Dictionary, Error, Packet, Rational, codec, decoder, encoder, format, frame, log, media,
    picture,
};
...
const DEFAULT_X264_OPTS: &str = "crf=28";
...
impl Transcoder {
    fn new(
        ...
        let mut encoder = codec::context::Context::new_with_codec(codec.ok_or(Error::InvalidData)?)
            .encoder()
            .video()?;
...
pub fn transcode(input_file: &Path, output_file: &Path) -> Result<(), Error> {
    init_ffmpeg();
    ...
    format::context::input::dump(&ictx, 0, Some(&input_file.display().to_string()));
        ...
        if ist_medium == media::Type::Video {
            ...
            transcoders.insert(
                ...
                Transcoder::new(
                    ...
                    &mut octx,
                    ost_index as _,
                    parse_opts(DEFAULT_X264_OPTS.to_string()).unwrap(),
    ...
    format::context::output::dump(&octx, 0, Some(&output_file.display().to_string()));
